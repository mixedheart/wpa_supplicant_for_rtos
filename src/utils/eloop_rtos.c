...
struct eloop_data {
	...
	OS_EVENTGROUP* eloop_events_group;
			     ...
			     void *eloop_data, void *user_data)
{
	...
	tmp[eloop.reader_count].event = BIT(sock);
	eloop.reader_count++;
	eloop.readers = tmp;
...
void eloop_run(void)
{
	...
	while (!eloop.terminate &&
		...
		if (ret == WAIT_TIMEOUT)
			continue;
#endif		
		for(i = CONFIG_MAX_READ_SOCKS; i<CONFIG_MAX_READ_SOCKS+CONFIG_MAX_EVENTS; i++)
		{
			if(events&BIT(i))
			{
				uint32_t idx = i-CONFIG_MAX_READ_SOCKS;
				eloop.events[idx].handler(eloop.events[idx].eloop_data, eloop.events[idx].user_data);
			}
		}
